...
| AR-006 | high | docs/dev-surface | `v2/dev/release.py`, `scripts/dev/retrofx-v2 status`, internal-alpha package manifests | machine-readable status still implied broader alpha by exposing `ready_for_controlled_alpha=true` even though the evidence only supported internal alpha | narrow the release-status surface, add explicit broader-alpha false gates, and document the distinction | yes | completed |
| AR-007 | medium | environment-specific | TWO-28 forced Wayland GNOME and Plasma plan runs | non-validated Wayland desktop sessions needed an explicit export-only warning rather than silent degradation | add explicit Wayland desktop warnings and keep those environments out of broader-alpha claims | yes | completed |
