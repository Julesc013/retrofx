...
- `scripts/dev/retrofx-v2 package-alpha` now builds a reproducible non-public internal-alpha package under `v2/releases/internal-alpha/<package-id>/`; it does not imply a broader-alpha package surface
...
- `v2/dev/release.py` is now the code-side source of truth for the current internal-alpha version, local tag candidate name, and release-status metadata surfaced through `status`, `package-alpha`, and `diagnostics`
...
- [ALPHA_BLOCKERS.md](ALPHA_BLOCKERS.md) and [ALPHA_READINESS.md](ALPHA_READINESS.md) now record the current readiness decision: internal-use yes, controlled internal alpha yes, broader-alpha no
...
- [ALPHA_READINESS.md](ALPHA_READINESS.md) now explicitly says internal-alpha continuation yes, local alpha-candidate yes, broader-testing no, and `ALPHA_CANDIDATE_READY=yes`
