...
| full 2.x suite after TWO-32 candidate prep | repo-local dev | `./v2/tests/test.sh` | suite remains green after adding the narrower technical-beta surface | `Ran 143 tests in 2.754s` and `OK` | pass | Final suite pass confirms the candidate wrapper and copied-toolchain package did not regress the implemented branch. |
