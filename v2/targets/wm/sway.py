"""sway WM fragment compiler for the early 2.x WM family."""
...
class SwayCompiler:
    ...
    def compile(self, resolved_profile: Mapping[str, Any], profile_output_root: Path) -> TargetCompileResult:
        ...
        warnings.extend(_wm_fragment_warnings(context))
        ...
        return TargetCompileResult(
            ...
            warnings=warnings,
            notes=[
                "Deterministic sway theme fragment from the resolved WM theme context.",
                "This output is config/theme generation only; it does not imply Wayland-wide render or compositor ownership.",
            ],
        )
    ...
    def _render(self, context: Any) -> str:
        ...
        lines = [
            "# RetroFX 2.x experimental WM target: sway",
            "# Include this fragment from a sway config that you own.",
            "# This file does not claim compositor-effect or global render ownership.",
        ...
        if context.ui_font:
            lines.append(f"font {context.ui_font} 10")
        if context.gaps > 0:
            lines.extend(
                [
...
def _wm_fragment_warnings(context: Any) -> list[str]:
    warnings: list[str] = []
    if context.icon_theme:
        warnings.append("sway fragments do not emit icon-theme hints in TWO-10.")
    if context.cursor_theme:
        warnings.append("sway fragments do not emit cursor-theme hints in TWO-10.")
    if context.launcher_style != "minimal":
        warnings.append("sway fragments do not represent launcher styling hints in TWO-10.")
    if context.notification_style != "minimal":
        warnings.append("sway fragments do not represent notification styling hints in TWO-10.")
    return warnings
