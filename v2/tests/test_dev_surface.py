...
class UnifiedDevSurfaceTests(unittest.TestCase):
    ...
    def test_status_report_runs(self) -> None:
        with TemporaryDirectory() as tmphome:
            ...
            self.assertIn("proposed_technical_beta_version", payload["release_status"])
            self.assertIn("proposed_technical_beta_status_label", payload["release_status"])
            self.assertIn("proposed_technical_beta_tag_name", payload["release_status"])
            self.assertIn("technical_beta_candidate_tag_name", payload["release_status"])
            self.assertIn("technical_beta_candidate_tag_exists", payload["release_status"])
            self.assertIn("technical_beta_candidate_tag_points_at_head", payload["release_status"])
            self.assertIn("technical_beta_candidate_tag_state", payload["release_status"])
            self.assertIn("latest_existing_local_technical_beta_tag", payload["release_status"])
            self.assertIn("technical_beta_support_matrix", payload["release_status"])
            self.assertIn("technical_beta_limitations", payload["release_status"])
            ...
            self.assertFalse(payload["release_status"]["needs_public_surface_hardening"])
            self.assertEqual(payload["release_status"]["public_surface_position"], "limited-public-technical-beta-candidate")
            self.assertEqual(payload["release_status"]["proposed_technical_beta_version"], "2.0.0-techbeta.1")
            ...
            self.assertEqual(payload["release_status"]["current_build_kind"], "technical-beta-candidate-prep")
    ...
    def test_machine_readable_summary_mentions_expected_commands(self) -> None:
        ...
        self.assertIn("package-technical-beta", command_names)
        ...
        self.assertTrue(payload["release_status"]["ready_for_limited_public_technical_beta"])
        self.assertEqual(payload["release_status"]["public_beta_blockers"], [])
